...
lazy_static! {
    ...
    static ref DRIVER: BtDriver<'static, Ble> = BtDriver::new(QWE.lock().unwrap(), None).unwrap();
}
...
fn main() -> Result<()> {
    ...
    // let bt_modem = unsafe { BluetoothModem::new() };
    // let driver: BtDriver<Ble> = BtDriver::new(bt_modem, None)?;

    // Obtain gatt and gap instances
    // let gatt = Arc::new(EspGatts::new(driver.borrow())?);
    let gap = Arc::new(EspBleGap::new((*DRIVER).borrow())?);
